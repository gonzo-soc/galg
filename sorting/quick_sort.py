...
def quick_sort(in_array, left, right):
    ...
    less_stable_nodes = filterLessStableNode(in_array, sn)
    higher_stable_nodes = filterHigherStableNode(in_array, sn)
    equal_stable_nodes = filterEqualStableNode(in_array, sn)
...
def filterLessStableNode(in_array, sn):
    return [i for i in in_array[0:] if i < sn]


def filterHigherStableNode(in_array, sn):
    return [i for i in in_array[0:] if i > sn]


def filterEqualStableNode(in_array, sn):
    return [i for i in in_array[0:] if i == sn]
...
out_array = []
sorted_array = quick_sort(in_array, 0, len(in_array))
